...
let EmotionShow = computed(() => store.state.EmotionShow)
let _EmotionList = ['微笑', '撇嘴', '色', '发呆', '得意', '流泪', '害羞', '闭嘴', '睡', '大哭',
    ...
    '激动', '街舞', '献吻', '左太极', '右太极']
let EmotionList = ref(_EmotionList);
let emit = defineEmits(['AppendInputValue'])

const ClickEmoticon = (EmoticonNo: number) => {
    store.commit('ChangeEmotionShow', false);
    store.commit('AppendMessageText', '[[' + EmotionList.value[EmoticonNo] + ']]');
    
    emit('AppendInputValue')
}
const OpenEmotion = (Value: boolean) => {
